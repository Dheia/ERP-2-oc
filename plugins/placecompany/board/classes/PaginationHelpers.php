...
{
    /**
     * Board 워드프레스 게시판 페이지 출력 함수
     * @link www.cosmosfarm.com
     * @copyright Copyright 2019 Cosmosfarm. All rights reserved.
     * @license http://www.gnu.org/licenses/gpl.html
     */
    public static function board_pagination($current_page, $total, $limit, $args=array()){
        if(!is_array($args)){
            $args = array();
        }
        ...
        foreach($args as $key=>$value){
            if(is_array($value)){
                $query_strings[] = http_build_query(array(Helpers::sanitize_key($key)=>$value));
            }
            else if($key == 'mod'){
                ...
                $query_strings[] = Helpers::sanitize_key($key).'='.urlencode(Helpers::board_htmlclear($value));
            }
        }
        ...
        if($current_page != 1){
            ...
            $paging = "<li class=\"first-page\"><a href=\"?pageid=1{$query_strings}\">First</a></li>" . "<li class=\"prev-page\"><a href=\"?pageid={$prev_page}{$query_strings}\">«</a></li>{$paging}";
        }
        if($current_page != $total_page){
            $next_page = $current_page + 1;
            $paging = "{$paging}<li class=\"next-page\"><a href=\"?pageid={$next_page}{$query_strings}\">»</a></li>" . "<li class=\"last-page\"><a href=\"?pageid={$total_page}{$query_strings}\">Last</a></li>";
        }
