...
class BoardFileHandler {
    ...
    function checkPath($path, $mk=true){
        ...
        if($mk) \File::makeDirectory($path, 0755, true, true);
        if(!file_exists($path)){
            return false;
        }
    ...
    function upload($name, $extension=array(), $limit_file_size=0){
         ...
         */
        if(!$this->path) {
            Flash::error("디렉토리 경로가 없거나 하위 디렉토리에 쓰기 권한이 없습니다.");
            redirect()->back(); exit;
        }
        ...
        if(count($extension)<=0 || !is_array($extension)){
            $extension = Helpers::board_allow_file_extensions(true);
        }
        \Event::fire('placecompany.board.board_upload_extension', [$extension]);
        ...
        if($limit_file_size){
            ...
            $this->limit_file_size = Helpers::board_limit_file_size();
        }
    ...
    private function singleUpload($file){
        ...
        if($file['size']){
            ...
            if(!is_uploaded_file($file['tmp_name'])){
                $this->rollback();
                $message = sprintf(Lang::get('placecompany.board::lang.:title 파일이 올바르게 업로드되지 않았습니다.', ['title' => $file['name']]));
                Flash::error($message);
                redirect()->back(); exit;
            }
            ...
            if(!$this->checkExtension($file['name'])){
                $this->rollback();
                $message = sprintf(Lang::get('placecompany.board::lang.:title 파일은 업로드 가능한 파일 형식이 아닙니다.', ['title' => $file['name']]));
                Flash::error($message);
                redirect()->back(); exit;
            }
            ...
            if(!$this->checkFileSize($file['size'])){
                $this->rollback();
                $message = sprintf(Lang::get('placecompany.board::lang.:title 파일의 용량이 너무 큽니다.', ['title' => $file['name']]));
                Flash::error($message);
                redirect()->back(); exit;
            }
            ...
            if($this->checkError($file['error'])){
                $this->rollback();
                $message = sprintf(Lang::get('placecompany.board::lang.:title 파일 업로드 중 오류가 발생했습니다.', ['title' => $file['name']]));
                Flash::error($message);
                redirect()->back(); exit;
            }
            ...
            \Event::fire('placecompany.board.board_uploaded_file', [array(
    ...
    private function multipleUpload($file){
        ...
        if(isset($files) && $files){
            foreach($files as $item){
                \Event::fire('placecompany.board.board_uploaded_file', [&$item, $this->name]);
                $new_files[] = $item;
            }
            return $new_files;
        }
...
//        if(Helpers::board_mime_type($image) == 'image/jpeg'){
