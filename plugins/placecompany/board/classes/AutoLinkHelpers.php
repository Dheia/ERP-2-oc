...
class AutoLinkHelpers
{
    public static function board_autolink($text){
        /*
         * Mark Goldsmith
         * http://css-tricks.com/snippets/php/find-urls-in-text-make-links/
         */
        return preg_replace_callback('#(?i)(http|https)?(://)?(([-\w^@]+\.)+(kr|co.kr|go.kr|net|org|edu|gov|me|com|co+)(?:/[^,\s]*|))#', 'board_autolink_prependHTTP', $text);
    }
    public static function board_autolink_prependHTTP($m){
        /*
         * Mark Goldsmith
         * http://css-tricks.com/snippets/php/find-urls-in-text-make-links/
         */
