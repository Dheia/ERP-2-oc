 ...
 */
class BoardTreeCategoryManager {
    ...
    public function setTreeCategory($tree_category){
        if(is_array($tree_category)){
            ...
            $this->tree_category = json_decode(htmlspecialchars_decode($tree_category), true);
        }
    }
        ...
        if($tree_category){
            foreach($tree_category as $key=>$value){
                $this->row .= '<li id="tree_category_'.$value['id'].'" style="display: list-item;">'.
                    '<div id="tree-category-'.$value['id'].'" class="menu-item-bar"><div data-id="'.$value['id'].'" class="menu-item-handle ui-sortable-handle" onclick="board_tree_category_edit_toggle(\''.$value['id'].'\', \''.$value['category_name'].'\', \''.$value['parent_id'].'\')">'.
                    ...
                    '<input type="hidden" id="tree-category-parent-'.$value['id'].'" class="board-tree-category-parents" name="tree_category['.$value['id'].'][parent_id]" value="'.$value['parent_id'].'">'.
                    '</div></div>';
     ...
     * @return array $search_option
     */
    public function getSelectedList(Request $request){
        $board_search_option = $request->get('board_search_option');
        $search_option = array();
        if(isset($board_search_option['tree_category_1']['value']) && $board_search_option['tree_category_1']['value']){
            for($i=1; $i<=$this->getCount(); $i++){
                if(!(isset($board_search_option['tree_category_'.$i]['value']) && $board_search_option['tree_category_'.$i]['value'])) break;
                $search_option[] = $board_search_option['tree_category_'.$i]['value'];
            }
        }
     ...
     * @return number
     */
    public function getCategoryNameWithDepth($depth, Request $request){
        $board_search_option = $request->get('board_search_option');;
        $current_category = isset($board_search_option['tree_category_'.$depth]['value'])?$board_search_option['tree_category_'.$depth]['value']:'';
